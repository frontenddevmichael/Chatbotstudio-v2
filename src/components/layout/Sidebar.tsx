...
const Sidebar = () => {
        ...
        {links.map(({ to, icon: Icon, label }) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center ${isTablet ? 'justify-center rounded-[8px] p-2.5' : 'gap-2.5 rounded-[6px] px-3 py-[7px]'} text-[14px] font-medium transition-colors ${
                active
                  ? 'bg-[hsl(var(--color-surface-2))] text-foreground'
                  : 'text-muted-foreground hover:bg-[hsl(var(--color-surface-1))] hover:text-foreground'
              }`}
            >
              <Icon className={`h-4 w-4 ${active ? 'text-primary' : ''} ${isTablet ? '' : ''}`} />
              {!isTablet && label}
            </Link>
          );
        })}
  ...
  return (
    <>
      {/* Mobile toggle — hidden since BottomTabBar handles mobile */}
      <button
        onClick={() => setOpen(!open)}
        className="fixed left-3 top-3 z-50 rounded-[6px] p-2 text-foreground lg:hidden hidden"
