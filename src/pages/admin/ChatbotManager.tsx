...
const ChatbotManager = () => {
  ...
  const deleteChatbot = useMutation({
    ...
    onError: () => { toast.error('Failed to delete'); setDeleteConfirm(null); },
  });

  const exportCSV = () => {
    const rows = filtered.map((b: any) => ({
      Name: b.name, Owner: owners?.[b.user_id] ?? 'Unknown', Status: b.is_active ? 'Active' : 'Inactive',
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-foreground">All Chatbots ({filtered.length})</h1>
                ...
                <tr key={bot.id} className="border-b border-border last:border-0 hover:bg-muted/30 cursor-pointer" onClick={() => setDetailBot(bot)}>
                  ...
                  <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                    <div className="flex items-center gap-2">
                      <button onClick={() => toggleActive.mutate({ id: bot.id, is_active: bot.is_active })} className="text-xs font-medium text-primary hover:underline">{bot.is_active ? 'Deactivate' : 'Activate'}</button>
                      ...
                      <button onClick={() => setDeleteConfirm({ id: bot.id, name: bot.name })} className="text-xs text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        <DialogContent className="max-w-md">
          ...
          {detailBot && (
            <div className="space-y-3 text-sm">
              ...
              <div className="rounded-md bg-muted p-3"><p className="text-xs text-muted-foreground">Embed Token</p><p className="font-mono text-xs text-foreground break-all">{detailBot.embed_token}</p></div>
            </div>
          )}
        </DialogContent>
      </Dialog>
