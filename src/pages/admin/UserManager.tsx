...
const UserManager = () => {
  ...
  const toggleAdmin = useMutation({
    ...
    onError: () => { toast.error('Failed'); setRoleConfirm(null); },
  });
  ...
  return (
    <AdminLayout>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="font-display text-2xl font-bold text-foreground">Users ({filtered.length})</h1>
                    ...
                    <tr key={u.id} className="border-b border-border last:border-0 hover:bg-muted/30 cursor-pointer" onClick={() => setDetailUser(u)}>
                      ...
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => resetMessages.mutate(u.id)} className="text-muted-foreground hover:text-foreground" title="Reset messages"><RotateCcw className="h-3.5 w-3.5" /></button>
                          ...
                          <button onClick={() => setDeleteConfirm({ id: u.id, name: u.full_name || 'Unnamed' })} className="text-xs text-muted-foreground hover:text-destructive"><Trash2 className="h-3.5 w-3.5" /></button>
                        </div>
                      </td>
                    </tr>
                  );
