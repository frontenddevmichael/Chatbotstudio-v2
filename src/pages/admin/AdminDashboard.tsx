...
import { Users, Bot, MessageSquare, AlertTriangle } from 'lucide-react';

const AdminDashboard = () => {
  const { data: stats } = useQuery({
    queryKey: ['admin-stats'],
    queryFn: async () => {
      const [{ count: userCount }, { count: botCount }, { count: convoCount }, { data: settings }] = await Promise.all([
        supabase.from('profiles').select('*', { count: 'exact', head: true }),
        supabase.from('chatbots').select('*', { count: 'exact', head: true }),
        supabase.from('conversations').select('*', { count: 'exact', head: true }),
        supabase.from('platform_settings').select('*').single(),
      ]);
      return { userCount: userCount ?? 0, botCount: botCount ?? 0, convoCount: convoCount ?? 0, settings: settings };
    },
  });
