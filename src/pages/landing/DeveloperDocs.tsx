...
const DeveloperDocs = () => {
  ...
  return (
    <section id="developers" ref={ref} className="relative py-28 px-6 bg-black">
      <div className="max-w-5xl mx-auto">
        ...
        >
          {STEPS.map((step, i) => (
            <div key={step.num} className="flex items-start gap-3">
              <span className="font-serif text-[28px] italic text-white/15 leading-none shrink-0">
                {step.num}
              </span>
              <div>
                <p className="text-[14px] font-medium text-white/80">{step.title}</p>
                <p className="text-[12px] text-white/35 mt-0.5 leading-relaxed">{step.desc}</p>
              </div>
              {i < 2 && <ArrowRight className="hidden sm:block h-4 w-4 text-white/10 mt-1 ml-auto shrink-0" />}
            </div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          className="rounded-[16px] border border-white/[0.06] bg-white/[0.02] overflow-hidden"
        >
          {/* Tab bar */}
          ...
          <div className="relative">
            <pre className="p-5 text-[12px] leading-[1.7] text-white/60 font-mono overflow-x-auto whitespace-pre">
              {SNIPPETS[active]}
            </pre>
            <button
              onClick={copy}
              className="absolute top-3 right-3 flex items-center gap-1.5 rounded-[8px] bg-white/[0.06] hover:bg-white/[0.1] px-3 py-1.5 text-[11px] font-medium text-white/50 hover:text-white/80 transition-colors"
            >
